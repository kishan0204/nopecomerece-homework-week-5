Feature: Demo Nopcommerce's topmenu acceptance criteria
  ...
  Background: User do on given URL
    When User type URl https://domo.nopcommarce.com
  ...
  Scenario: Verify that user can see tho Book category page with filters and list of products
    When User in on given URL
  ...
  Scenario: : verify that user can See 'Name: A do Z' is first option in 'Sort by' filter
  Given User is on Books Category page
  When User is on Books Category page.
    And Click on 'Sort by' filter
    And check that 'Nane: A to Z is first in order
